	...
	public void insertProduct(Scanner sc) { //1
		...
		try {
			...
			if (c > 0) {
				...
				sql = "INSERT INTO MENU_INFO VALUES (?, SEQ_MENUNO.NEXTVAL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
				ps = con.prepareStatement(sql);
	...
	public void updateProduct(Scanner sc) { //2
		...
		try {
			...
			if (c > 0) {
				...
				int m_no = sc.nextInt();
				sql = "SELECT * FROM MENU_INFO WHERE MENU_NO = " + m_no;
				ps = con.prepareStatement(sql);
				int p = ps.executeUpdate();
				if (p > 0) {
					ps.close();
	...
	public void deleteProduct(Scanner sc) { //3
		...
		try {
			...
			if (c > 0) {
				...
				int m_no = sc.nextInt();
				sql = "DELETE FROM MENU_INFO WHERE MENU_NO = " + m_no;
				ps = con.prepareStatement(sql);
				int p = ps.executeUpdate();
				if (p > 0) {
		...
		try {
			...
			while (rs.next()) {
				mcdmenuVO = new McdMenuVO(rs.getString("MENU_CAT"), rs.getInt("MENU_NO"), rs.getString("MENU_NM"), 
						rs.getInt("MENU_PR"), rs.getInt("MENU_WG"), rs.getInt("MENU_ML"), rs.getDouble("MENU_K"), rs.getDouble("MENU_SU"), 
						rs.getDouble("MENU_D"), rs.getDouble("MENU_F"), rs.getDouble("MENU_N"), rs.getDouble("MENU_C"), rs.getString("MENU_DC"));
